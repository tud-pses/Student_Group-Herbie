...
// class for wrapping functions and local variables for saving features
class node_class
{
public:
    node_class();
...
node_class::node_class()
{
    ...
    cv_ptr = nullptr;
    _firstImage =1;
    float camera_parameters[10] = {540.68603515625, 0.0, 479.75, 0.0, 540.68603515625, 269.75, 0.0, 0.0, 1.0};  // from topic camera info qhd
    cameraMatrix = Mat(3, 3, CV_32F, camera_parameters);
    poses.push_back(Mat::eye(4,4,CV_64F));  //initialize Pose (0 0 0 0 0 0)
...
void node_class::imageCallback(const sensor_msgs::Image::ConstPtr& imagMsg)
{
    ...
    */
    ...
    //Mat r;
    //Rodrigues(R,r); rotation vector ( axis)
    Mat R_t,R_t2;
    hconcat(R,t,R_t);
    ...
    // baue t_f
    //drawing the estimated positions (x,z) on a plane
    char text[100];
